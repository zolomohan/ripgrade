 ...
 * control on this page that arrives in the operating system's own blue with the
 * operating system's own corners, and a table of them under a card built out of
 * hairlines and rounded rectangles reads as a form that wandered in. This is the
 * app's own square — the surface and the line every other control stands on,
 * filled with the foreground colour when it is ticked.
 *
...
function Tick({
  ...
  return (
    <label
      title={
        refusal ?? (disabled ? undefined : "Shift-click to tick a run of tracks")
      }
      onMouseDown={(e) => {
            ...
            {preferredChosen.length > 0 && (
              <p className="mb-3 rounded-control border border-red-500/40 bg-red-500/[0.08] px-3 py-2 text-red-700 dark:text-red-300">
                {preferredChosen
                  ...
                  .join(", ")}{" "}
                {preferredChosen.length === 1 ? "is" : "are"} among the languages
                you keep
